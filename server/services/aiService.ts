...
export async function processNaturalLanguageOrder(
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content: 
            "You are an expert restaurant order parser. Extract menu items, quantities, and special instructions from natural language orders."
        },
        {
          role: "user",
          content: 
            `Parse the following customer order into structured data: "${orderText}"
             ...
             If an item isn't on the menu, try to find the closest match.
             Format your response as a JSON object with this structure: 
    ...
    const content = response.choices[0].message.content || "{}";
    return JSON.parse(content) as { items: any[], notes: string };
  } catch (error) {
